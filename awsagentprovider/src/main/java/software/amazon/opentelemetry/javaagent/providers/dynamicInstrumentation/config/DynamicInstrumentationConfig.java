...
public final class DynamicInstrumentationConfig {
   ...
   * &rarr; {@code ec2:default}). This makes DI self-sufficient — the value used as the request's
   * Environment lookup key matches Application Signals without relying on the agent proxy to inject
   * it.
   *
  ...
  public String getDeploymentEnvironment() {
    ...
    // "ec2:default" is also the fallback when the resource is still empty (no cloud.platform /
    // host / k8s attributes yet), so don't cache that case — the Resource may still be populating.
    if (!env.isEmpty()
        && (!"ec2:default".equals(env) || EnvironmentResolver.hasPlatformContext(resource))) {
